...
def product():
    return Product(
        ...
        unit_price=1,
        supplier="test",
        supplier_address=Address("test", "test", "test", "test"),
        delivery_address=Address("test", "test", "test", "test"),
...
@pytest.mark.skipif(os.getenv("BREACT_API_KEY") is None, reason="BREACT_API_KEY not set")
def test_emission_factor_reject_result(product):
    breact_categorizer = BreactCategorizer(os.getenv("BREACT_API_KEY"))
    ...
    mock_emission_factors_fetcher.fetch_emission_factor_from_query.return_value = EmissionFactor(
        ...
        name="test",
        description=(
            "Emission intensity of supply chain (with margins i.e. cradle to shelf) in US dollars spend on: "
            "cable and subscription programming. This factor is representative of the described commodity "
            "(equivalent to a goods or services category) and was obtained from the USEEIO model v1.0.1. Refer "
            "to the source for the source-specific data quality assessment. Retrieved from Supply Chain Factors Dataset v1.1.1"
        )
    )
